...
class MemoryManager(IMemoryStore):
    ...
    async def retrieve_by_session(self, session_id: str, top_k: int = 5) -> list[MemoryEntry]:
        """
        Retrieve memories scoped to a specific session.

        Uses STM's session buffer (which indexes by session_id) and
        LTM's retrieve_by_session (which filters by session_id column).
        """
        # STM treats query as session_id (by design)
        stm_results = await self._stm.retrieve(session_id, top_k=top_k)

        if len(stm_results) >= top_k:
            return stm_results[:top_k]

        # Fill from LTM (session-scoped query)
        remaining = top_k - len(stm_results)
        ltm_results = await self._ltm.retrieve_by_session(session_id, top_k=remaining)
