...
class LLMProvider(str, Enum):
    LOCAL  = "local"   # vLLM server — primary and only required provider
    OPENAI = "openai"  # Optional: GPT-4 research baseline (requires API key)
...
class Settings(BaseSettings):
    ...
    # --- Local vLLM (primary and only required provider) ---
    local_vllm_base_url: str = "http://localhost:8100/v1"
    local_vllm_model: str = "Qwen/Qwen2.5-7B-Instruct"
    local_vllm_served_name: str = "chainmind-qwen"
    local_vllm_enabled: bool = True
    local_vllm_gpu_memory_util: float = 0.92
    local_vllm_max_model_len: int = 16384
    local_vllm_prefix_caching: bool = True

    # --- LLM Router (local-first, local-only by default) ---
    llm_primary_provider: LLMProvider = LLMProvider.LOCAL
    llm_fallback_chain: list[LLMProvider] = Field(
        default_factory=lambda: [
            LLMProvider.LOCAL,   # vLLM — fast, free, GPU-local
            # LLMProvider.OPENAI, # Uncomment only for GPT-4 research baselines
        ]
    )
