...
class BaseAgent(IAgent):
    ...
    async def process(self, task: TaskRequest, context: AgentContext) -> TaskResponse:
        ...
        try:
            while step_count < self._max_steps:
                ...
                # === OBSERVE ===
                result_str = str(tool_result.result) if tool_result.success else tool_result.error
                ...
                observation = (
                    ...
                    else f"Tool '{tool_name}' FAILED: {result_str}"
                )
                ...
                # === FORCE CONVERGENCE ===
                # If we've used most of our budget, force a final answer on next step
                if step_count >= self._max_steps - 2 and tool_result.success:
                    # We have tool results — synthesize a final answer now
                    logger.info(f"Force-convergence at step {step_count}: synthesizing answer from tool results")
                    all_results = "\n".join(
                        s.tool_output or s.content
                        for s in reasoning_trace
                        if s.step_type in (ReActStep.OBSERVE,) and s.tool_output
                    )
                    final_answer = f"Based on the tool results:\n{all_results[:2000]}"
                    break

            if final_answer is None:
                # Last resort: synthesize from whatever observations we have
                observations = [
                    s.tool_output or s.content
                    for s in reasoning_trace
                    if s.step_type == ReActStep.OBSERVE and s.tool_output
                ]
                if observations:
                    final_answer = f"Partial results from analysis:\n" + "\n".join(obs[:500] for obs in observations)
                else:
    ...
    def _build_think_prompt(
        ...
        # Show full content for tool/observation messages, truncate user/assistant
        history_parts = []
        for m in messages[-6:]:
            if m.role == "tool":
                # Tool results must be visible for the agent to synthesize answers
                history_parts.append(f"[{m.role}]: {m.content[:1500]}")
            else:
                history_parts.append(f"[{m.role}]: {m.content[:400]}")
        history = "\n".join(history_parts)

        # Urgency instruction for later steps
        urgency = ""
        if step >= self._max_steps - 3:
            urgency = (
                "\n\n⚠️ IMPORTANT: You are running low on steps. "
                "You MUST provide a FINAL_ANSWER now using whatever information you have gathered so far. "
...
3. Call a tool ONLY ONCE per step. After receiving results, provide FINAL_ANSWER.
4. If a tool already returned useful results (shown in history), do NOT call it again. Extract the fact from the result as FINAL_ANSWER.
5. ACTION_INPUT must be valid JSON on a single line, e.g. {{"query": "my search"}}
...
If you have the final answer from the tool, respond EXACTLY in this format:
THOUGHT: [your reasoning based strictly on tool output]
FINAL_ANSWER: [your complete answer to the user's question based strictly on tool output]{urgency}

Respond now:"""

    def _format_tool_descriptions(self) -> str:
        """Format available tools as compact name + description (no JSON schemas).

        Full parameter schemas bloat the prompt by ~4000 tokens across 13 tools.
        The agent only needs name + description to decide which tool to call;
        the full schema is used at execution time, not reasoning time.
        """
        ...
        for name, (_, tool_def) in self._tool_registry.items():
            required = ", ".join(tool_def.required_params) if tool_def.required_params else "none"
            descriptions.append(
                f"- {name}: {tool_def.description} (required args: {required})"
            )
        return "\n".join(descriptions)
    ...
    def _parse_tool_call(self, thought: str) -> tuple[str, dict[str, Any]] | None:
        ...
        Handles multi-line JSON by collecting all text after ACTION_INPUT:
        and extracting the first valid JSON object using brace matching.
        """
        try:
            ...
            if "ACTION_INPUT:" in action_part:
                ...
                # Remove newlines from tool name (model may add extra whitespace)
                tool_name = tool_name.split("\n")[0].strip()
                args_str = action_part.split("ACTION_INPUT:", 1)[1].strip()

                # Extract JSON object using brace matching (handles multi-line)
                args = self._extract_json_object(args_str)
            else:
    ...
    def _extract_json_object(text: str) -> dict[str, Any]:
        """Extract the first valid JSON object from text, handling multi-line."""
        # Try direct parse first (single-line case)
        ...
        # Brace-matching approach for multi-line JSON
        brace_depth = 0
        start_idx = text.find("{")
        if start_idx == -1:
        ...
        # Fallback: couldn't find matching braces
        return {"input": text.strip()[:200]}
...
What went wrong and what should be done differently? Be concise."""
    ...
    async def _load_memory(self, session_id: str) -> str:
        ...
        if self._memory_store is None:
            return ""
        try:
            # Use session-scoped retrieval (not keyword search)
            if hasattr(self._memory_store, 'retrieve_by_session'):
                entries = await self._memory_store.retrieve_by_session(session_id, top_k=5)
            else:
