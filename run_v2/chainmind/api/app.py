...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        molecular_server = MolecularMCPServer()
        kb_server = KnowledgeBaseMCPServer(knowledge_base=state.knowledge_base)
        research_server = ResearchMCPServer()

        # Initialize Specialist Agents — each gets ONLY its relevant MCP servers
        ...
        specialist_agents = [
            ComputationalChemistAgent(
                ...
                mcp_servers=[molecular_server, kb_server],  # Chemistry + KB only
            ...
            WebResearchAgent(
                llm_router=state.llm_router,
                mcp_servers=[research_server, kb_server],  # Research + KB only
                guardrails=guardrails,
                memory_store=state.memory_manager,
                max_steps=settings.max_agent_steps,
            ),
            KnowledgeGraphAgent(
                llm_router=state.llm_router,
                mcp_servers=[research_server, kb_server],  # Research (for KG tool) + KB only
                guardrails=guardrails,
                memory_store=state.memory_manager,
                max_steps=settings.max_agent_steps,
            ),
